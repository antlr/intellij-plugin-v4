...
public class MyPsiUtils {
     ...
     */
    @Nullable
	private static PsiElement findFirstChildOfType(final PsiElement parent, final TokenSet types){
	 ...
	 */
	private static PsiElement[] collectNodesWithName(@Nullable PsiElement root, final String tokenText) {
		return PsiTreeUtil.collectElements(root, element -> {
	...
	private static PsiElement[] collectChildrenOfType(PsiElement root, final IElementType tokenType) {
	...
	private static PsiElement[] collectChildrenWithText(PsiElement root, final String text) {
		List<PsiElement> elems = new ArrayList<>();
		for (PsiElement child : root.getChildren()) {
	...
	private static PsiElement findElement(PsiElement startNode, int offset) {
